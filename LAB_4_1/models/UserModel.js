...
//izbrisiKorisnikaVjezba
/*
dbDeleteUser = static async(user) => {
    const sql = `delete from users where users.user_name = user.user_name`;
    try {
        await db.query(sql, []);
    } catch(err) {
        console.log('Ne mogu izbrisati korisnika iz sustava');
        console.log(err);
