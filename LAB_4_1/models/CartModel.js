...
async function removeAllItems(cart, id) {
...
//za eventualni unos u tablicu cart u bazi
/*
persist = static async (user, id,items) => {
        const sql = `Insert into cart(user.id, id, items)`;
        try {
            const result = await db.query(sql, []);
...
/*
removeFromCart = static async (user, id) =>{
    const sql = `delete from cart where cart.user_id === user.id 
    && id === cart.inventory_id`;
    try {
            const result = await db.query(sql, []);
            //return result.rows[0].id;
        } catch(err) {
